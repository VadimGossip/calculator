...
func (r *repository) Get(ctx context.Context, token string) (*domain.Token, error) {
	...
	if err := r.db.QueryRowContext(ctx, selectStmt, token).Scan(&t.Id, &t.UserId, &t.Token, &t.ExpiresAt); err != nil {
		return nil, err
	}
	return &t, nil
}
